const http = require('http');  //commonJS规范 通过js去实现,为了实现模块化的开发
const jquery = require('jquery');  //node_modules里面
const fs = require('fs');
const urlModel = require('url');//能够把url分割为路径,查询信息,hash...
const qs = require('querystring');//能够url序列化操作
            ...
            case '/add':
                ...
                fs.mkdir('www/'+mkdirname+'/',(err)=>{
                    if (err) {
                            if (err.code === 'EEXIST') {
                             //说明重名了
                             fs.readdir('www',(error,fileAry)=>{
                                 console.log(fileAry);//返回的是一个数组,数组中放的是当前文件下的所有文件名字
                                 //['js','js(1)']

                                let num = 0;
                                let b = fileAry.includes(mkdirname);
                                let name = '';
                                while(b){
                                    //js(1) -> js -> js(num)
